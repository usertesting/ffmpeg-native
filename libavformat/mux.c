...
#define RET(x,y) x //static int RET(int v, int l) { fprintf(stderr, "%d: returning %d\n", l, v); return v; }
int av_interleaved_write_frame(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (pkt) {
        ...
        if (st->codec->codec_type == AVMEDIA_TYPE_AUDIO && pkt->size == 0)
            return RET(0,__LINE__);//0;

        av_dlog(s, "av_interleaved_write_frame size:%d dts:%s pts:%s\n",
                pkt->size, av_ts2str(pkt->dts), av_ts2str(pkt->pts));
        if ((ret = compute_pkt_fields2(s, st, pkt)) < 0 && !(s->oformat->flags & AVFMT_NOTIMESTAMPS))
            return RET(ret,__LINE__);//ret;

        if (pkt->dts == AV_NOPTS_VALUE && !(s->oformat->flags & AVFMT_NOTIMESTAMPS))
            return RET(EINVAL,__LINE__);//AVERROR(EINVAL);
    } else {
    ...
    for (;; ) {
        ...
        if (ret < 0)
            return RET(ret,__LINE__);//ret;
        if(s->pb && s->pb->error)
            return RET(s->pb->error,__LINE__);//s->pb->error;
    }
}
